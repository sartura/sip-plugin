...
typedef int (*sr_callback)(ctx_t *, sr_change_oper_t, char *, char *, char *,
                           sr_val_t *);

int sysrepo_option_callback(ctx_t *ctx, sr_change_oper_t op, char *xpath,
                            char *ucipath, char *key, sr_val_t *val) {
  int rc = SR_ERR_OK;
...
int sysrepo_boolean_callback(ctx_t *ctx, sr_change_oper_t op, char *xpath,
                             char *ucipath, char *key, sr_val_t *val) {
  int rc = SR_ERR_OK;
  ...
  if (val->data.bool_val) {
    ...
    snprintf(password_xpath, XPATH_MAX_LEN,
...
int sysrepo_section_callback(ctx_t *ctx, sr_change_oper_t op, char *xpath,
                             char *ucipath, char *key, sr_val_t *val) {
  int rc = SR_ERR_OK;
...
int sysrepo_list_callback(ctx_t *ctx, sr_change_oper_t op, char *orig_xpath,
                          char *orig_ucipath, char *key, sr_val_t *val) {
  int rc = SR_ERR_OK;
  size_t count = 0;
  bool enabled = true;
...
int sysrepo_list_callback_enable(ctx_t *ctx, sr_change_oper_t op, char *xpath,
                                 char *orig_ucipath, char *key, sr_val_t *val) {
  int rc = SR_ERR_OK;
  struct uci_ptr ptr = {};
  char ucipath[] = "voice_client.direct_dial.direct_dial";
  ...
  if (SR_OP_CREATED == op || SR_OP_MODIFIED == op) {
    if (false == val->data.bool_val) {
      ...
      return sysrepo_list_callback(
          ctx, op, xpath, "voice_client.direct_dial.direct_dial", key, val);
    }
  } else if (SR_OP_DELETED == op) {
    // TODO
  }
...
static sr_uci_link table_sr_uci[] = {
    {sysrepo_section_callback, false, "voice_client.%s",
     "/terastream-sip:sip/sip-account[account='%s']"},
    {sysrepo_option_callback, false, "voice_client.%s.name",
     "/terastream-sip:sip/sip-account[account='%s']/account_name"},
    {sysrepo_option_callback, false, "voice_client.%s.displayname",
     "/terastream-sip:sip/sip-account[account='%s']/display_name"},
    {sysrepo_boolean_callback, true, "voice_client.%s.enabled",
     "/terastream-sip:sip/sip-account[account='%s']/enabled"},
    {sysrepo_option_callback, false, "voice_client.%s.domain",
     "/terastream-sip:sip/sip-account[account='%s']/domain"},
    {sysrepo_option_callback, false, "voice_client.%s.user",
     "/terastream-sip:sip/sip-account[account='%s']/username"},
    {sysrepo_option_callback, false, "voice_client.%s.secret",
     "/terastream-sip:sip/sip-account[account='%s']/password"},
    {sysrepo_option_callback, false, "voice_client.%s.authuser",
     "/terastream-sip:sip/sip-account[account='%s']/authentication_name"},
    {sysrepo_option_callback, false, "voice_client.%s.host",
     "/terastream-sip:sip/sip-account[account='%s']/host"},
    {sysrepo_option_callback, false, "voice_client.%s.port",
     "/terastream-sip:sip/sip-account[account='%s']/port"},
    {sysrepo_option_callback, false, "voice_client.%s.outboundproxy",
     "/terastream-sip:sip/sip-account[account='%s']/outbound/proxy"},
    {sysrepo_option_callback, false, "voice_client.%s.outboundproxyport",
     "/terastream-sip:sip/sip-account[account='%s']/outbound/port"},
    {sysrepo_option_callback, false, "voice_client.SIP.rtpstart",
     "/terastream-sip:sip/advanced/rtpstart"},
    {sysrepo_option_callback, false, "voice_client.SIP.rtpend",
     "/terastream-sip:sip/advanced/rtpend"},
    {sysrepo_option_callback, false, "voice_client.SIP.dtmfmode",
     "/terastream-sip:sip/advanced/dtmfmode"},
    {sysrepo_list_callback, false, "voice_client.direct_dial.direct_dial",
     "/terastream-sip:sip/digitmap/dials"},
    {sysrepo_list_callback_enable, false, "voice_client.direct_dial.XXXX",
     "/terastream-sip:sip/digitmap/enabled"},
};
...
static int parse_uci_config(ctx_t *ctx, char *key) {
  ...
  for (int i = 0; i < n_mappings; i++) {
    ...
    if (0 == strncmp("voice_client.direct_dial.", ucipath,
                     strlen("voice_client.direct_dial."))) {
      rc = UCI_ERR_NOTFOUND;
    } else {
  ...
  if (SR_ERR_OK == rc && NULL != secret) {
    CHECK_RET(rc, cleanup, "failed to get asterisk secret: %s",
              sr_strerror(rc));
...
int sysrepo_to_uci(ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
...
void ubus_cb(struct ubus_request *req, int type, struct blob_attr *msg) {
  ...
  for (int i = 0; i < counter; i++) {
    ...
    snprintf(xpath, XPATH_MAX_LEN,
             "/terastream-sip:sip-state[account='%s']/refresh", key);
    rc = sr_val_set_xpath(&sr_val[sr_item], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    ...
    snprintf(xpath, XPATH_MAX_LEN,
             "/terastream-sip:sip-state[account='%s']/state", key);
    rc = sr_val_set_xpath(&sr_val[sr_item], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    rc = sr_val_set_str_data(&sr_val[sr_item], SR_STRING_T,
                             (char *)json_object_get_string(v));
    ...
    snprintf(xpath, XPATH_MAX_LEN,
             "/terastream-sip:sip-state[account='%s']/registered", key);
    rc = sr_val_set_xpath(&sr_val[sr_item], xpath);
    CHECK_RET(rc, cleanup, "failed sr_val_set_xpath: %s", sr_strerror(rc));
    bool registered =
        (0 == strcmp("Registered", json_object_get_string(v)) ? true : false);
...
int fill_state_data(ctx_t *ctx, char *xpath, sr_val_t **values,
                    size_t *values_cnt) {
  int rc = SR_ERR_OK;
  uint32_t id = 0;
  ...
  u_rc = ubus_invoke(u_ctx, id, "registry_status", buf.head, ubus_cb, &ubus_ctx,
                     0);
  if (UBUS_STATUS_OK != u_rc) {
    ERR("ubus [%d]: no object asterisk\n", u_rc);
    rc = SR_ERR_INTERNAL;
